...
import com.lrpc.loadbalance.ConsistentHashLoadBalance;
...
public class ReferenceProxyHandler implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		LRPCRequest request = LRPCRequest.builder()
			...
			.payload(payload)
			.build();
		LRPCBootstrap.getInstance().requestCache.set(request);
		if (!LRPCBootstrap.getInstance().CACHE_SERVICE_LIST.containsKey(interfaceConsumer.getName())) {
			synchronized (this) {
				if (!LRPCBootstrap.getInstance().CACHE_SERVICE_LIST.containsKey(interfaceConsumer.getName())) {
					LRPCBootstrap.getInstance()
						.CACHE_SERVICE_LIST
						.put(interfaceConsumer.getName(), registry.lookup(interfaceConsumer.getName()));
				}
			}
		}
		...
		if (!LRPCBootstrap.getInstance().CACHE_INTERFACE_LOADBALANCE.containsKey(interfaceConsumer.getName())) {
			synchronized (this) {
				if (!LRPCBootstrap.getInstance().CACHE_INTERFACE_LOADBALANCE.containsKey(interfaceConsumer.getName())) {
					LRPCBootstrap.getInstance()
						.CACHE_INTERFACE_LOADBALANCE
						.put(interfaceConsumer.getName(), new ConsistentHashLoadBalance(100, addresses));
//							.put(interfaceConsumer.getName(),new RoundRobinLoadBalance(addresses));
				}
			}
		}
		AbstractLoadBalancer balance = LRPCBootstrap.getInstance().CACHE_INTERFACE_LOADBALANCE.get(interfaceConsumer.getName());
		InetSocketAddress address = balance.choiceNode();
		...
		LRPCBootstrap.getInstance().requestCache.remove();
